...
public class PostController {
  ...
  @PostMapping("/posts")
  public ResponseEntity<BaseResponse<PostResponse>> createPost(@Parameter(description = "게시글 작성 내용")
  ...
  @PutMapping("/posts/{id}")
  public ResponseEntity<BaseResponse<PostResponse>> updatePost(@Parameter(description = "게시글 수정 내용")
  ...
  public ResponseEntity<BaseResponse<Boolean>> deletePost(@Parameter(description = "특정 게시글 ID")
  @PathVariable Long id) {
   ...
   * 게시글 조회 많은순 조회 API
   */
  @Operation(summary = "게시글 조회수 많은순 조회",
      description = "게시판 페이지에서 조회 많은순으로 조회할 때 요청되는 API")
  @GetMapping("/posts/popular")
  public ResponseEntity<BaseResponse<List<PostResponse>>> getPostByViewCount() {
    List<PostResponse> responses = postService.getPostsByViewCount();
    return ResponseEntity.ok(BaseResponse.success("게시글 조회수 많은순 조회 성공", responses));
  }
  ...
  public ResponseEntity<BaseResponse<List<PostResponse>>> getRecentPosts() {
    List<PostResponse> responses = postService.getRecentPosts();
    return ResponseEntity.ok(BaseResponse.success("게시글 최신순 조회 성공", responses));
  }
}

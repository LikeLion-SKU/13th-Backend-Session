...
public class PostController {
  ...
  public ResponseEntity<BaseResponse<PostResponse>> getPostById(
      @Parameter(description = "특정 게시글 ID") @PathVariable(value = "id") Long id) {
  ...
  @Operation(summary = "게시글 최신순 조회",
    description = "게시판에 올려진 게시글을 최신순으로 조회하는 API")
  @GetMapping("/posts/latest")
  public ResponseEntity<BaseResponse<List<PostResponse>>> getPostsOrderByCreatedAtDesc() {
    List<PostResponse> responses = postService.getPostsOrderByCreatedAtDesc();
    return ResponseEntity.ok(BaseResponse.success(responses));
  }

  @Operation(summary = "게시글 조회수순 조회",
      description = "게시판에 올려진 게시글을 조회 많은 순으로 조회하는 API")
  @GetMapping("/posts/popular")
  public ResponseEntity<BaseResponse<List<PostResponse>>> getPostsOrderByViewCountDesc() {
    List<PostResponse> responses = postService.getPostsOrderByViewCountDesc();
    return ResponseEntity.ok(BaseResponse.success(responses));
  }
}
